...
# =========================================================
# 3. DATABASE (SELF-REPAIR LOGIC ADDED)
# =========================================================
...
def init_db():
    ...
    # --- SELF-REPAIR: Check if unit_cost column exists in an old database ---
    try:
        db.execute("SELECT unit_cost FROM assets LIMIT 1")
    except sqlite3.OperationalError:
        # If it fails, the column is missing. Add it!
        db.execute("ALTER TABLE assets ADD COLUMN unit_cost REAL DEFAULT 0")
        
    try:
        db.execute("SELECT cost FROM logs LIMIT 1")
    except sqlite3.OperationalError:
        db.execute("ALTER TABLE logs ADD COLUMN cost REAL DEFAULT 0")
...
# ---------------------------------------------------------
if mode == "📊 Stock":
    # Use a safe query that works even if the DB was just repaired
